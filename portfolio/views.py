...
def client_list(request):
    ...
    if lname:
        clients = clients.filter (lname=lname)
...
def trackdonations(request, pk):
    ...
    return render(request, 'portfolio/trackdonations.html', {'donations': donations})

@staff_member_required
def trackdonations2(request, pk):
    donations = Donation.objects.filter(donor=pk)
    return render(request, 'portfolio/trackdonations2.html', {'donations': donations})
